  ...
  it('compile form and test required fields', () => {
    cy.clearLocalStorage().should((ls) => {
      cy
        ...
        .get('input[name=height]').type(allFields.height)
        .get('form').submit()
        .get(`li[data-key="${allFields.gender}"]`).should('include.text', allFields.gender)
        .get(`li[data-key="${allFields.age}"]`).should('include.text', allFields.age)
        .get(`li[data-key="${allFields.weight}"]`).should('include.text', allFields.weight)
        .get(`li[data-key="${allFields.height}"]`).should('include.text', allFields.height)
    })
  })
})
